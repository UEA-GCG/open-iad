...
def parse_arguments_main():
    ...
    parser.add_argument('--dataset', '-d', type=str, default='mvtec2d',
                        choices=['_example', 'mvtec2d', 'mvtec3d', 'mpdd', 'mvtecloco', 'mtd',
                                 'btad', 'mvtec2df3d', 'visa', 'dagm', 'coad', 'small_tool'])
    parser.add_argument('--model', '-m', type=str, default='patchcore',
                        choices=['_example', '_patchcore', 'patchcore', 'csflow', 'dne',
                                 'draem', 'igd', 'cutpaste', 'devnet', 'dra', 'favae', 'padim',
                                 'reverse', 'spade', 'fastflow', 'softpatch', 'cfa', 'stpm',
                                 'simplenet', 'softpatch'])
    parser.add_argument('--net', '-n', type=str, default='wide_resnet50',
                        choices=['net_example', 'wide_resnet50', 'resnet18', 'net_csflow',
                                 'vit_b_16', 'net_draem', 'net_dra', 'net_igd',
                                 'net_reverse', 'net_favae', 'net_fastflow', 'net_cfa', 'net_devnet',
                                 'vig_ti_224_gelu'])
    ...
    # vanilla
    parser.add_argument('--vanilla', '-v', action='store_true', default=True)  # 批量测试
    ...
    # data augmentation type 数据增强方法 dataholder.py调用
    parser.add_argument('--train-aug-type', '-tag', default='normal',
                        choices=['normal', 'cutpaste', 'small_tool'], help='data augmentation type')
    parser.add_argument('--valid-aug-type', '-vag', default='normal',
                        choices=['normal', 'cutpaste', 'small_tool'], help='data augmentation type')
