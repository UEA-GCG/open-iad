...
__all__ = ['Small_Tool', 'small_tool_classes']

# GUO:根据实际类别设置
def small_tool_classes():
    return ["bottle", "cable", "grid", "metal_nut", "screw", "toothbrush", "zipper"]


class Small_Tool(Dataset):
    def __init__(self, data_path, learning_mode='centralized', phase='train', 
                 data_transform=None, num_task=15, skip_load_info=False):

        self.data_path = data_path
        self.learning_mode = learning_mode
        self.phase = phase
        self.class_name = small_tool_classes()
        self.img_transform = data_transform[0]
        self.mask_transform = data_transform[1] 
        assert set(self.class_name) <= set(small_tool_classes())
        ...
        # load dataset
        if not skip_load_info:  # 小工具训练阶段即跳过测试样本的预加载数据集信息
            self.load_dataset()   # 遍历文件夹读文件名
            self.allocate_task_data()  # 打乱顺序
    ...
    def load_dataset(self):
        # input x= img paths/class, label y=[0, 1], good is 0 and bad is 1, mask is ground truth
            ...
            for class_name in class_in_task:
                ...
                for img_type in img_types:
                    ...
                    if not os.path.isdir(img_type_dir):
                        continue
                    img_path_list = sorted([os.path.join(img_type_dir, f) for f in os.listdir(img_type_dir)
                                            if f.endswith('.png')])
