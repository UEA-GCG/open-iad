...
class PatchCore(torch.nn.Module):
    ...
    def _embed(self, images, detach=True, provide_patch_shapes=False):
        ...
        # [[4,512,32,32],[4,1024,16,16]]
        features = [features[layer] for layer in self.layers_to_extract_from]

        # [([4,1024,512,3,3],[32,32]),([4,256,1024,3,3],[16,16])]
        features = [
    ...
    # 跳过重新从训练数据读的步骤
    def init_fit(self):
        self.anomaly_scorer.fit(detection_features=[self.sampled_features])

    # 持久化抽样数据
    def save_data(self, filepath):
        np.save(filepath, self.sampled_features)

    # 加载抽样数据
    def load_data(self, filepath):
        self.sampled_features = np.load(filepath)
    ...
    def _fill_memory_bank(self, input_data):
        ...
        self.sampled_features = features  #保存数据下来
