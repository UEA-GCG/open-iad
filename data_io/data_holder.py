...
class DataHolder(object):
    def __init__(self, dataset_class, config):
        ...
        self.valid_dataset = dataset_class(data_path=self.config['data_path'],  # 在线测试时图片不使用文件夹存储
                                           learning_mode=self.config['learning_mode'],
                                           phase='test',
                                           data_transform=valid_data_transform,
                                           num_task=self.config['num_task'])#,
                                           #skip_load_info=self.config['online'])

        self.refer_dataset = extract_fewshot_data(self.train_dataset, self.config['ref_num'])  # only for model 'dra'
        self.vis_dataset = extract_fewshot_data(self.valid_dataset, self.config['vis_num'])  # 可视化使用
